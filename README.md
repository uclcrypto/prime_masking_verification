...
## Getting Started
To reproduce the provided examples, the verification tools [SILVER](https://github.com/Chair-for-Security-Engineering/SILVER), [VERICA](https://github.com/Chair-for-Security-Engineering/VERICA), [PROLEAD](https://github.com/ChairImpSec/PROLEAD) and [MATCHI](https://github.com/cassiersg/matchi) are required. We do not aim at providing (or maintaining) comprehensive platform-independent installation guidelines for these tools here, those should be found in their original repositories. However, since the instructions provided in some of the original repositories are already outdated (and sometimes not easily compatible with newer utility versions anymore), we provide one exemplary installation guideline at the end of this README, tested at the time of writing this on Ubuntu Server 24.04 LTS.

Once all tools and utilities are installed in the same location (if they are not in the same directory their paths need to be adjusted in the Makefile), you may clone this repository from said location (```git clone https://github.com/uclcrypto/prime_masking_verification```), enter its folder (```cd prime_masking_verification```) and call ```make all```. If all paths are correctly annotated, this should prompt four consecutive verification runs, one with each tool.
...
Calling ```make gadget_simu_prolead``` prompts a simulation-based verification using PROLEAD of a pre-synthesized netlist (based on the NanGate 45nm library) of a squaring gadget corresponding to the Makefile parameters **SHARES**, **BITS**, **INPUT_DISTRI** and **VERSION**. This verification is not exhaustive but can provide empirical evidence for the (glitch+transition-robust) bit-level probing security of gadgets with parameters for which formal verification is computationally infeasible.

Calling ```make compo_formal_matchi``` prompts the synthesis of a full [small-pSquare](https://github.com/uclcrypto/small-pSquare) cipher implementation corresponding to the Makefile parameters **SHARES** and **VERSION** using yosys followed by composition-level formal verification using MATCHI of the (glitch+transition-robust) probing security of the whole circuit under the assumption that the underlying squaring gadget satisfies glitch-robust t-PINI. The output denotes whether verification has been successful ("Verification successful.") or not.

## Selecting Valid Parameters
The parameter **SHARES** takes integer values 2, 3 or 4 and denotes the number of shares in the masked implementation. More than four shares are currently not implemented.

The parameter **BITS** takes integer values 2, 3 or 5 for formal verification using SILVER/VERICA, or 2, 3, 5 or 7 for simulation-based verification using PROLEAD. It denotes the Mersenne prime exponent * *n* * in * *p = 2^n-1* * and therefore also the field/word size of the underlying arithmetic.
...
To repeat, we do not aim at providing (or maintaining) comprehensive platform-independent installation instructions for the used verification tools. However, the following set of commands has been successfully tested on Ubuntu Server 24.04 LTS at the time of writing.
